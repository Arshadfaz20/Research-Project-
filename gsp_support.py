...
def calculate_energy_pct(dfd, dfc):
    ...
    cons1 = dfd[dfd.columns.values].sum()
    cons2 = dfc[dfc.columns.values].sum()

    ax1.pie(cons1.values, autopct='%1.1f%%', startangle=90)
    ax2.pie(cons2.values, autopct='%1.1f%%', startangle=90)
    first_legend = ax1.legend(dfd.columns, loc = 'lower right', bbox_to_anchor=(.5, -.4), fontsize = 8)
    second_legend = ax2.legend(dfc.columns, loc = 'lower right', bbox_to_anchor=(.5, -.4), fontsize = 8)
